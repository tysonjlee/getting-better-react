...
function NoteCardDialogTimestamp({ id }) {
  ...
  function determineTimestamp() {
    if (note.isDeleted) return "Deleted " + convertTimestamp(note.deletedAt) // If deleted
    else if (note.wasUpdated) return "Edited " + convertTimestamp(note.updatedAt) // If edited 
